  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (exists) {
          results.failed++;
          results.errors.push({ row: rowNum, email, reason: "Ye email already registered hai" });
          continue;
        }

        // ── 3. Subscription plan ──
        const planRaw   = row.subscriptionPlan?.toString().trim().toLowerCase();
        const validPlan = ["basic", "premium", "gold"].includes(planRaw) ? planRaw : null;
        ...
        if (row.productTitle?.toString().trim()) {
          ...
          if (!categoryDoc) {
            ...
            await Product.create({
              ...
              description: row.description?.toString().trim() || "Bulk uploaded product",
              images:      [],
              status:      prodStatus,
              isActive:    true,
            });
  ...
  try {
    ...
    const templateData = [
      ...
      // Notes row
      {
        ...
        annualTurnover:     "Below 1 Crore / 1-5 Crore / 5-10 Crore / 10-50 Crore / 50+ Crore",
    ...
    res.setHeader("Content-Disposition", "attachment; filename=bulk_sellers_template.xlsx");
