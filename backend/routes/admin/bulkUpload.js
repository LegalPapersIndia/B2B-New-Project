...
router.post("/bulk-upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    // ── Saari categories + subcategories + plans ek baar DB se load karo (performance) ──
    const allCategories = await Category.find({});
    const allSubCategories = await SubCategory.find({});
    const allPlans = await Plan.find({});

    // ✅ NEW — plan key → duration (days) ka quick lookup map, DB se
    const planDurationMap = {};
    allPlans.forEach((p) => {
      planDurationMap[p.key] = p.duration;
    });
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        // ── 3. Subscription plan ──
        const planRaw = row.subscriptionPlan?.toString().trim().toLowerCase();
        const validPlan = ["silver", "gold", "diamond"].includes(planRaw)
          ? planRaw
          : null;
        // ✅ UPDATED — duration ab DB (Plan collection) se aayega, hardcoded nahi
        const planDays = validPlan ? (planDurationMap[validPlan] ?? 30) : 0;
...
isPhoneVerified: true,
        ...
        if (row.productTitle?.toString().trim()) {
          ...
          if (!categoryDoc) {
            ...
            const slug = await generateSlug(row.productTitle.toString().trim());
            const prodStatus = validPlan ? "approved" : "pending";
            // ✅ UPDATED — featured field hata diya, kyunki Product model mein ab ye field hai hi nahi
  ...
  try {
    ...
    const templateData = [
      ...
      // Notes row
      {
        ...
        pincode: "6 digit",
        address: "",
        subscriptionPlan: "silver / gold / diamond / blank (no subscription)",
        productTitle: "Blank = no product",
        categoryName: `Exactly: ${catNames || "Admin se check karo"}`,
        subCategoryName: `Exactly: ${subNames || "Admin se check karo"}`,
        price: "Number only",
