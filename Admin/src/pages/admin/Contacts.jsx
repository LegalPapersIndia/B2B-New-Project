...
export default function Contacts() {
  ...
  const handleDelete = (contact) => {
    setConfirmModal({
      ...
      message: `Are you sure? Contact from "${contact.name}" will be permanently deleted.`,
      confirmText: "Delete",
      showCancel: true,
      onConfirm: async () => {
        try {
            ...
            <tbody>
              ...
              {!loading && contacts.map((contact) => (
                ...
                >
                  ...
                  {/* EMAIL */}
                  <td className="p-4 text-white/60 text-xs">{contact.email}</td>

                  {/* MOBILE */}
                  <td className="p-4 text-white/60">{contact.mobile}</td>

                  {/* SUBJECT */}
                  <td className="p-4 text-white/70">{contact.subject}</td>

                  {/* STATUS */}
                  <td className="p-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${statusStyle(contact.status)}`}>
                      {contact.status}
                    </span>
                  </td>
          ...
          <div className="bg-[#0D0D14] border border-white/10 rounded-2xl w-full max-w-xl overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${statusStyle(selectedContact.status)}`}>
