...
import { FaArrowLeft, FaTrash, FaTimes, FaCheckCircle, FaExclamationTriangle } from "react-icons/fa";
    ...
    <div className="min-h-screen bg-[#0A0A0F] text-white p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0D0D14] border border-white/10 rounded-2xl p-4 sm:p-6">
          ...
          {product.images?.[0]?.url && (
            <img
              ...
              className="w-full h-64 object-contain bg-white/5 rounded-xl mb-4 cursor-pointer hover:opacity-90 transition"
            />
          )}
            ...
            <div className="flex gap-3 flex-wrap">
              {product.images.map((img, i) => (
                <img
                  ...
                  className="h-16 w-16 object-cover rounded-lg border border-white/10 cursor-pointer hover:opacity-80 transition"
                />
              ))}
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          {/* MAIN INFO */}
          <div className="bg-[#0D0D14] border border-white/10 rounded-2xl p-4 sm:p-6 space-y-4">
          ...
          <div className="bg-[#0D0D14] border border-white/10 rounded-2xl p-4 sm:p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p className="text-white/40 text-xs mb-1">Subscription</p>
                <p className={`font-medium text-xs flex items-center gap-1 ${product.seller?.subscriptionActive ? "text-green-400" : "text-yellow-400"}`}>
                  {product.seller?.subscriptionActive ? <FaCheckCircle /> : <FaExclamationTriangle />}
                  {product.seller?.subscriptionActive ? "Active" : "Pending"}
                </p>
              </div>
              <div>
          ...
          <button
            ...
            disabled={deleting}
            className="w-full bg-red-700 hover:bg-red-800 disabled:opacity-50 py-3 rounded-xl text-sm font-semibold transition flex items-center justify-center gap-2"
          >
            <FaTrash className="text-xs" /> {deleting ? "Deleting..." : "Delete Product"}
          </button>
      ...
      {selectedImage && (
        ...
        >
          <div
            className="relative max-w-md sm:max-w-lg max-h-[70vh] flex items-center justify-center"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute -top-10 right-0 text-white bg-white/10 hover:bg-white/20 rounded-full w-9 h-9 flex items-center justify-center text-lg font-bold transition z-10"
            >
              <FaTimes />
            </button>
            <img
