...
const ALL_COLS = [...REQUIRED_COLS, "moq", "unit", "brand", "stock", "shortdesc", "keyfeatures"];
...
const BulkUploadProduct = () => {
  ...
  const downloadTemplate = () => {
    ...
    const headers = [
      ...
      "MOQ", "UNIT", "BRAND", "STOCK", "SHORTDESC", "KEYFEATURES",
    ];
    const hints = [
      ...
      "Key features, separate each with a new line / Alt+Enter",
    ];
    const sample1 = [
    ...
    // Column widths
    ws["!cols"] = [30, 22, 24, 12, 55, 10, 14, 18, 10, 35, 40].map((w) => ({ wch: w }));
    ...
    const infoData = [
      ...
      ["🔵 Optional Fields:", "MOQ, UNIT, BRAND, STOCK, SHORTDESC, KEYFEATURES"],
      [""],
      ["⚠️ Rules:"],
      ...
      ["", "• KEYFEATURES — write each feature on a new line (use Alt+Enter in Excel)"],
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < updatedRows.length; i++) {
      ...
      try {
        ...
        if (row.keyFeatures) formData.append("keyFeatures", row.keyFeatures);
