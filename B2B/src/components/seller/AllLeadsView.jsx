  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-md border border-gray-100 overflow-hidden">
        <div className="bg-gradient-to-r from-orange-600 to-orange-500 text-white px-4 sm:px-6 py-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <FaClipboardList className="shrink-0" />
            <span className="font-semibold truncate">Verify Leads</span>
            <span className="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full shrink-0">{requirements.length}</span>
          </div>
          <button onClick={() => setActiveTab("verify")} className="text-white/70 hover:text-white text-xs underline transition whitespace-nowrap shrink-0">
            View All →
          </button>
        </div>
        {requirementsLoading ? (
      ...
      {/* DIRECT LEADS PREVIEW */}
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-md border border-gray-100 overflow-hidden">
        <div className="bg-gradient-to-r from-blue-900 to-blue-800 text-white px-4 sm:px-6 py-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <FaInbox className="shrink-0" />
            <span className="font-semibold truncate">Direct Leads</span>
            <span className="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full shrink-0">{leads.length}</span>
          </div>
          <button onClick={() => setActiveTab("leads")} className="text-white/70 hover:text-white text-xs underline transition whitespace-nowrap shrink-0">
            View All →
          </button>
        </div>
        {leadsLoading ? (
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 sm:p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><p className="text-gray-400 text-xs mb-1">Buyer</p><p className="font-medium">{selectedLead.buyerName}</p></div>
                <div><p className="text-gray-400 text-xs mb-1">Phone</p><a href={`tel:${selectedLead.buyerPhone}`} className="font-medium text-green-600">{selectedLead.buyerPhone}</a></div>
                <div><p className="text-gray-400 text-xs mb-1">Email</p><a href={`mailto:${selectedLead.buyerEmail}`} className="font-medium text-blue-600 text-xs break-all">{selectedLead.buyerEmail || "—"}</a></div>
                <div><p className="text-gray-400 text-xs mb-1">Quantity</p><p className="font-medium">{selectedLead.quantity || "—"}</p></div>
                <div><p className="text-gray-400 text-xs mb-1">Status</p>
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${getStatusStyle(selectedLead.status)}`}>
                    {selectedLead.status}
