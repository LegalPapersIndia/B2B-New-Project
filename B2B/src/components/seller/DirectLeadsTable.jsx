...
const DirectLeadsTable = ({
  ...
  const getLeadPageNumbers = () => {
    const pages = [];
    const delta = 1;

    for (let i = 1; i <= leadTotalPages; i++) {
      if (
        i === 1 ||
        i === leadTotalPages ||
        (i >= leadPage - delta && i <= leadPage + delta)
      ) {
        pages.push(i);
          ...
          {leadTotalPages > 1 && (
            <div className="px-6 py-4 border-t flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setLeadPage((p) => Math.max(p - 1, 1))} disabled={leadPage === 1}
                  className="px-4 py-2 rounded-xl border border-gray-200 text-gray-600 hover:border-blue-800 hover:text-blue-800 transition disabled:opacity-40 disabled:cursor-not-allowed">
                  ← Prev
                </button>
                {/* ✅ UPDATED - ab getLeadPageNumbers() se limited pages + ellipsis render honge */}
                {getLeadPageNumbers().map((p, idx) =>
                  p === "..." ? (
                ...
                <button onClick={() => setLeadPage((p) => Math.min(p + 1, leadTotalPages))} disabled={leadPage === leadTotalPages}
                  className="px-4 py-2 rounded-xl border border-gray-200 text-gray-600 hover:border-blue-800 hover:text-blue-800 transition disabled:opacity-40 disabled:cursor-not-allowed">
                  Next →
                </button>
              </div>
