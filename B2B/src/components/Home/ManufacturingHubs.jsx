        ...
        >
          {cities.map((city, index) => (
            <SwiperSlide key={city._id || index}>
              ...
              <Link
                to={`/hub/${city.slug}`}
                className="block bg-white rounded-xl shadow-sm hover:shadow-lg transition-all overflow-hidden border border-gray-100 group"
              >
                {/* IMAGE */}
                {/*  UPDATED — height chhoti */}
                <div className="h-36 sm:h-40 overflow-hidden relative">
                ...
                <div className="p-3">
                  ...
                  {/* ✅ UPDATED — ab div hai (parent card hi Link hai, nested <a> avoid karne ke liye) */}
                  <div
                    className="w-full bg-orange-600 group-hover:bg-blue-800 text-white py-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-all duration-300"
                  >
                    Explore Hub <ArrowRight className="w-3.5 h-3.5" />
                  </div>
                </div>
