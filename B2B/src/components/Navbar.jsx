...
import { Link, useNavigate } from "react-router-dom";
import { getSearchSuggestions } from "../api/productApi";
import { getNavbarSettings } from "../api/navbarSettingsApi";
import { getFooterSettings } from "../api/footerSettingsApi";

// ─────────────────────────────────────────
// FESTIVAL BADGE
// ─────────────────────────────────────────
const FestivalBadge = ({ navbar }) => {
  const [show, setShow] = useState(false);

  useEffect(() => {
    if (!navbar?.festivalActive || !navbar?.festivalName) return;

    const now = new Date();
    const start = navbar.festivalStartDate
      ? new Date(navbar.festivalStartDate)
      : null;
    const end = navbar.festivalEndDate
      ? new Date(navbar.festivalEndDate)
      : null;
  ...
  return (
    <>
      <span
        className="text-xs font-bold px-2.5 py-1 rounded-full border animate-pulse whitespace-nowrap"
        style={{
          color: navbar.festivalTextColor,
          borderColor: navbar.festivalTextColor + "40",
          backgroundColor: navbar.festivalTextColor + "15",
        }}
      >
        {navbar.festivalEmoji} {navbar.festivalName}
      </span>
    </>
  );
};
...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchNavbar = async () => {
      try {
        ...
        if (res.data) setNavbar(res.data);
      } catch (err) {
    ...
    const detectLocation = async () => {
      ...
      try {
        const res = await fetch(
          "http://ip-api.com/json/?fields=regionName,city,status",
        );
        const data = await res.json();
        if (data.status === "success") {
          setDetected({ region: data.regionName || "", city: data.city || "" });
  ...
  // LOGO TEXT SPLIT
  const logoText = navbar?.logoText || "LPI-B2B";
  const logoTagline = navbar?.logoTagline || "Wholesale Marketplace";
  const logoFirst = logoText.includes("-")
    ? logoText.split("-")[0] + "-"
    : logoText;
  const logoSecond = logoText.includes("-") ? logoText.split("-")[1] : "";
    ...
    <header className="sticky top-0 z-50 w-full bg-white border-b border-gray-200 shadow-sm">
      ...
      <div className="bg-black text-gray-300 text-[10px] sm:text-xs">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-2 flex justify-between items-center">
          ...
          <div className="flex items-center justify-between w-full md:w-auto gap-4 md:gap-8">
            <span className="flex items-center gap-2">
              <FaPhoneAlt className="text-[#F54900]" />
              {navbar?.phone || "+91 75052 66931"}
            </span>
            <span className="flex items-center gap-2">
              <FaEnvelope className="text-[#F54900]" />
              {navbar?.email || "support@b2bhub.com"}
            </span>
          </div>
        </div>
      </div>
      ...
      <nav className="max-w-7xl mx-auto px-4 lg:px-8 py-4">
        <div className="flex flex-col gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-black">
                    {logoFirst}
                    <span className="text-[#1447E6]">{logoSecond}</span>
                  </h1>
                  {/* FESTIVAL BADGE */}
                  <FestivalBadge navbar={navbar} />
                </div>
                <p className="text-[11px] text-gray-500 -mt-1">{logoTagline}</p>
              </div>
            </Link>
          ...
          {/* MOBILE MENU */}
          <div
            className={`${isMenuOpen ? "flex flex-col gap-4" : "hidden"} lg:hidden bg-white p-5 border-t rounded-2xl`}
          >
            <div className="relative mb-2" ref={searchRef}>
              <input
                type="text"
